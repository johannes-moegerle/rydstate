...
def test_get_state_data_for_sqdt_alkali_state() -> None:
    ...
    assert row["underspecified_contribution"] == 0
...
def test_generate_states_table(species_specifier: str) -> None:
    ...
    contributions = table["underspecified_contribution"]
...
def test_generate_matrix_elements_table(species_specifier: str) -> None:
    ...
    if is_ion:
        ...
        reduced_reference = [total_charge * np.sqrt(2 * states[int(ids)].f_tot + 1) for ids in table["id_initial"]]
        np.testing.assert_allclose(table["val"], reduced_reference)
        ...
        for ids, val in zip(table["id_initial"], table["val"], strict=True):
            ...
            assert isinstance(state, RydbergStateSQDT)  # ions are always single channel quantum defect theory states
            for m in get_m_range(state.f_tot, None):
                state_m = RydbergStateSQDT(species, n=state.n, angular=state.angular.replace_m(m))
