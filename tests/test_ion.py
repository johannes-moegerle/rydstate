...
def test_strontium_ion_p_state_energies(n: int, energy_cm: float) -> None:
    ...
    angular = AngularKetLS(l_r=1, j_tot=0.5, f_tot=0.5, species="Sr88_ion")
    state = RydbergStateSQDT("Sr88_ion", n=n, angular=angular)
...
def test_strontium_ion_d_state_quantum_defects(n: int, quantum_defect: float) -> None:
    ...
    angular = AngularKetLS(l_r=2, j_tot=1.5, f_tot=1.5, species="Sr88_ion")
    state = RydbergStateSQDT("Sr88_ion", n=n, angular=angular)
...
def test_strontium_ion_s_state_quantum_defect() -> None:
    ...
    angular = AngularKetLS(l_r=0, j_tot=0.5, f_tot=0.5, species="Sr88_ion")
    state = RydbergStateSQDT("Sr88_ion", n=60, angular=angular)
    assert state.n - state.nu == pytest.approx(2.7062, abs=0.0005)
...
def test_electric_monopole_matrix_elements() -> None:
    ...
    """
    angular_s = AngularKetLS(l_r=0, j_tot=0.5, f_tot=0.5, m=0.5, species="Sr88_ion")
    angular_p = AngularKetLS(l_r=1, j_tot=1.5, f_tot=1.5, m=0.5, species="Sr88_ion")
    ion = RydbergStateSQDT("Sr88_ion", n=46, angular=angular_s)
    ion_p = RydbergStateSQDT("Sr88_ion", n=46, angular=angular_p)
    ion_s47 = RydbergStateSQDT("Sr88_ion", n=47, angular=angular_s)
    ...
    angular_rb = AngularKetLS(l_r=0, j_tot=0.5, f_tot=0.5, m=0.5, species="Rb")
    neutral = RydbergStateSQDT("Rb", n=46, angular=angular_rb)
...
def test_strontium_ion_nist_levels_match_quantum_defects() -> None:
    ...
    for (n, l_r, j_tot, s_tot), energy in levels.items():
        ...
        if j_tot == l_r + 0.5 and levels.get((n, l_r, l_r - 0.5, s_tot)) == energy:
            ...
            continue
        angular = AngularKetLS(l_r=l_r, j_tot=j_tot, f_tot=j_tot, species="Sr88_ion")
        nu_nist = sqdt.calc_nui(n, angular, use_nist_data=True)
        nu_qd = sqdt.calc_nui(n, angular, use_nist_data=False)
