...
def test_overlap_with_sqdt_state(basis: BasisMQDT) -> None:
    ...
    for n in range(30, 35):
        sqdt_state = RydbergStateSQDTDivalent(mqdt_state.species, n=n, l=0, s=1, j=1)
        overlaps[n] = abs(mqdt_state.calc_reduced_overlap(sqdt_state))

    best_n = max(overlaps, key=overlaps.__getitem__)
    best_sqdt = RydbergStateSQDTDivalent(mqdt_state.species, n=best_n, l=0, s=1, j=1)
...
def test_matrix_element_between_mqdt_and_sqdt_state(basis: BasisMQDT) -> None:
    ...
    p_sqdt = RydbergStateSQDTDivalent(s_mqdt.species, n=round(s_mqdt.nu) + 3, l=1, s=1, j=2)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Yb171", (0.5, 0.5), (1, 1), "P F=1/2, nu > 5.9"),
